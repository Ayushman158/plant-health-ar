...
export class DebugHud {
  ...
  update(app, nowMs) {
    ...
    const rows = [
      ...
      ['plantIdx', seg.plantIndex],
      ['modelSees', seg.dominantLabel],
      ['itsShare', seg.dominantShare ? seg.dominantShare.toFixed(3) : '0'],
      ['stalledMs', health ? health.stalledForMs : '—'],
      ['dropped', seg.droppedCallbacks],
      ['reloads', seg.reloads],
      ['inFlight', String(seg.inFlight)],
      ['—shape—', ''],
      ['areaShare', a?.shape ? a.shape.areaShare.toFixed(3) : '—'],
      ['compactness', a?.shape ? a.shape.compactness.toFixed(3) : '—'],
      ['fill', a?.shape ? a.shape.fill.toFixed(3) : '—'],
      ['—detection—', ''],
